...
MainLoop(){
	...
	Loop
	{
      ...
      ;Get one key at a time ; Waits for the user to type a string.
       ...
       ; ???????????????????
       ; ???????????????????
      ; ;19-02-12_20-00;;;Critical, On ; Wenn der erste Parameter fehlt (oder das Wort On ist), wird der aktuelle Thread als kritisch eingestuft; das heiÃŸt, dass dieser Thread nicht von anderen Threads unterbrochen werden kann. ; If the first parameter is omitted (or the word On), the current thread is made critical, meaning that it cannot be interrupted by another thread.
		EndKey := ErrorLevel
