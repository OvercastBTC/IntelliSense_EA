...
ReadInTheActionList(sql_template_dir, calledFromStr){ ;Read in the actionList
	...
	toolTipGui(RegExReplace(A_LineFile, ".*\\") ":" A_ThisFunc ":" A_LineNumber ,x:=0,,"_|",A_LineNumber,"Gray")  ; x will be offset if y is symbolic
...
ReadActionList( calledFromStr ){
	...
	if(!itsAGeneratedList){
		...
		if(fileEx){
            ...
            if(1 && g_config.debug.active && !InStr(actionList,"isNotAProject")){
...
; clipboard := g_actionListDB
	...
	DatabaseRebuilt := MaybeConvertDatabase(g_config["sql"]["template"]["dir"])
