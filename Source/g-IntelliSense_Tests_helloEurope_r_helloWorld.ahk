...
global closeAllOtherAHK_while_testing := false
...
if(closeAllOtherAHK_while_testing)
    closeAllOtherAHK()
exitFunc(){
    restoreClosedAHK()
...
MsgBox,262208,% ":)`n" A_ThisFunc ":" A_LineNumber " " RegExReplace(A_LineFile, ".*\\") ,% msg "`n`n(" A_ThisFunc ":" A_LineNumber " " RegExReplace(A_LineFile, ".*\\") ")"
...
tests(){
  ...
  ;  run,..\start.ahk
	ToolTip2sec( "Sleep,2000 `n(" A_ThisFunc " " RegExReplace(A_LineFile,".*\\") ":"  A_LineNumber ")" )
	Sleep,2000
  ;feedbackMsgBox("hi :) lets make some tests now. ", A_LineNumber . " , " . A_ScriptName,1,1)
	SetTitleMatchMode,2

	ToolTip2sec( "closeDisturbingWindows() `n(" A_ThisFunc " " RegExReplace(A_LineFile,".*\\") ":"  A_LineNumber ")" )
	feedbackMsgBox("closeDisturbingWindows() ", A_LineNumber . " , " . A_ScriptName,1,1)
...
;############## we inside: open_for_second_time_actionList_of_this_new_window
 ; next step open it again
	...
	{
      ;IfWinExist,.txt
		msgbox,% ">" closeAllOtherAHK_while_testing "< = closeAllOtherAHK_while_testing`n ???? (" A_ThisFunc " " RegExReplace(A_LineFile,".*\\") ":"  A_LineNumber ")"
		if(closeAllOtherAHK_while_testing)
    		WinClose,.ahk
		msgbox,% ">" tWL "< = close tWL`n ???? (" A_ThisFunc " " RegExReplace(A_LineFile,".*\\") ":"  A_LineNumber ")"
		WinClose,% tWL
		WinWaitClose,% tWL,1
		if(closeAllOtherAHK_while_testing){
			WinWaitClose,.ahk,,1
...
open_actionList_first_time( openInputBoxTitle ){
  ...
  ; quick and dirty
	if(address)
		Edit,% address ; todo: quick and dirty 31.03.2018 20:02 18-03-31_20-02
...
;/¯¯¯¯ winCloseTry ¯¯ 190131110039 ¯¯ 31.01.2019 11:00:39 ¯¯\
winCloseTry(t){
	if(!closeAllOtherAHK_while_testing){
		MsgBox,262208,% " winCloseTry(`n`n " t "`n`n) `n" A_ThisFunc ":" A_LineNumber " " RegExReplace(A_LineFile, ".*\\") ,% "Oops `n(" A_ThisFunc ":" A_LineNumber " " RegExReplace(A_LineFile, ".*\\") ")"
		return
	}
	SetTitleMatchMode,2
	Loop,9
...
;\____ winCloseTry __ 190131110045 __ 31.01.2019 11:00:45 __/
...
exitFunc(){__
    global closeAllOtherAHK_while_testing
    if(closeAllOtherAHK_while_testing){
...
closeAllOtherAHK(){
	...
	if(!closeAllOtherAHK_while_testing)
		MsgBox,262160,% " then it should not visit this function :(`n" A_ThisFunc ":" A_LineNumber " " RegExReplace(A_LineFile, ".*\\") ,% ":(`n(" A_ThisFunc ":" A_LineNumber " " RegExReplace(A_LineFile, ".*\\") ")"
